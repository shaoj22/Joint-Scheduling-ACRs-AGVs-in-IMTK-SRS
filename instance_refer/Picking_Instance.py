    ...
    def get_distance(self, idx1, idx2, extra=True):
        ...
        if extra:
            ...
            if x1 // (self.block_length + 1) == x2 // (self.block_length + 1):
                if (y1 == y2 or (
                        abs(y1 - y2) == 1 and (self.idx2type[idx1] == "aisle" or self.idx2type[idx2] == "aisle"))
            ...
            if y1 == y2:
                ...
                if self.idx2type[idx1] == "pod" and self.idx2type[
                    self.xy2idx[x1, y1 + 1]] == "aisle":  # aisle above idx1
                    dist += 2
                if self.idx2type[idx2] == "pod" and self.idx2type[
                    self.xy2idx[x2, y2 - 1]] == "aisle":  # aisle below idx2
                    dist += 2
            else:
                # idx1 lower than idx2
                if self.idx2type[idx1] == "pod" and self.idx2type[
                    self.xy2idx[x1, y1 - 1]] == "aisle":  # aisle below idx1
                    dist += 2
                if self.idx2type[idx2] == "pod" and self.idx2type[
                    self.xy2idx[x2, y2 + 1]] == "aisle":  # aisle above idx2
                    dist += 2
        return dist
        ...
        for i in range(self.nodeNum):
            for j in range(self.nodeNum):
                disMatrix[i, j] = self.map.get_distance(self.nodes[i]["pos_idx"], self.nodes[j]["pos_idx"],
                                                        extra=self.real_dist)
        # self.check_disMatrix(disMatrix)
        return disMatrix

    def check_disMatrix(self, disMatrix):
        # 检查距离矩阵是否满足三角不等式
        wrong_pairs = [(i, k, j) for i in self.N for j in self.N for k in self.N if
                       i != j and j != k and i != k and disMatrix[i, j] > disMatrix[i, k] + disMatrix[k, j]]
